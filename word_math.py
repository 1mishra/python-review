...
class WordMath():
    ...
    81
    >>> WordMath('nine hundred ninety nine').compute()
    999
	...
	hundred_words = 'hundred'.split()
	number_words = digit_words + tens_words + decades_words + hundred_words

	operator_words = {'add': '+', 'plus': '+', 'minus': '-', 'subtract': '-',
					  'times': '*', 'multiplied by': '*', 'divide': '/', 'divided by': '/',
					  'modulo': '%', 'power of': '**', 'exponential': '**'}
	...
	def symbolize_one_token(token: str) -> str:
		...
		#
		# elif token in WordMath.hundred_words:
		# 	return (WordMath.hundred_words.index(token) + 1) * 100
	...
	def symbolize(tokens):
        ...
        [3, '*', 5]
        >>> WordMath.symbolize(['add', 'plus', 'minus', 'subtract', 'times', 'divide', 'modulo', 'power'])
        ['+', '+', '-', '-', '*', '/', '%','**']
        """
		if isinstance(tokens, str):
			...
			if 'hundred' in tokens:
				tokens.remove('hundred')
				token_list.append(WordMath.symbolize_one_token(tokens[0]) * 100)
				for t in tokens[1:]:
	...
	def tokenize(s: str) -> list:
        ...
        >>> WordMath.tokenize('ten power three')
        ['ten', 'power of', 'three']
        """
		...
		while len(s):
			...
			if result:
				...
				if fw in WordMath.number_words or fw in WordMath.operator_words:
					...
					# if fw in WordMath.number_words:
					# 		d = dw
	...
	def compute(self):
		...
		for s in symbols:
			if isinstance(s, str):
				...
				current_value = self.calc_operation(current_value, operator, s)
				operator = None
...
if __name__ == '__main__':
	...
	print(WordMath('ninety nine multiplied by three times eleven').compute())
	print(WordMath('ninety nine divided by three times eleven').compute())
	print(WordMath('three exponential five').compute())

	print(WordMath('three times eleven').compute())
	print(WordMath('three power of five').compute())
	print(WordMath('ninety nine add eleven').compute())
	print(WordMath('nine hundred ninety nine').compute())

	print(WordMath('six hundred').compute())
